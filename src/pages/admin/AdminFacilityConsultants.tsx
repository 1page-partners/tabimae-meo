...
export default function AdminFacilityConsultants(){
  ...
  return <section className="role-page"><Link className="back" to={`/admin/facilities/${facilityId}`}><ArrowLeft size={15}/>施設詳細へ戻る</Link><header><span>ADMIN・担当管理</span><h1>{facility?.name??'施設'}のコンサルタント</h1><p>担当コンサルタントとメイン担当者を管理します。</p></header>
    <div className="inline-form"><label>追加するコンサルタント<select value={selected} onChange={event=>setSelected(event.target.value)}><option value="">選択してください</option>{available.map(item=><option value={item.id} key={item.id}>{item.name||item.email}</option>)}</select></label><label><input type="checkbox" checked={primary} onChange={event=>setPrimary(event.target.checked)}/>メイン担当者にする</label><button disabled={!selected||busy} onClick={()=>void assign()}><Plus size={15}/>担当に追加</button></div>
    {mutationError&&<p className="data-error">{mutationError.message}</p>}{error&&<p className="data-error">担当情報を取得できませんでした。</p>}
    <div className="data-table"><div className="data-row head"><span>氏名</span><span>メール</span><span>担当区分</span><span>操作</span></div>{isLoading?<p className="loading-row">読み込み中…</p>:assigned.map(item=><div className="data-row" key={item.consultantId}><strong>{item.name||'名称未設定'}</strong><span>{item.email||'メール未設定'}</span><span className="pill">{item.isPrimary?'メイン担当':'担当'}</span><div>{!item.isPrimary&&<button disabled={busy} onClick={()=>mutations.setPrimary.mutate(item.consultantId)}><Star size={14}/>メインにする</button>}<button disabled={busy} onClick={()=>remove(item.consultantId,item.name||item.email||'コンサルタント')}><Trash2 size={14}/>担当解除</button></div></div>)}{!isLoading&&!assigned.length&&<p className="loading-row">担当コンサルタントはまだ設定されていません。</p>}</div>
  </section>
}
