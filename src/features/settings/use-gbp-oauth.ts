...
export function useGBPOAuth(){
  ...
  const syncReviews=useMutation({mutationFn:async()=>{if(!facilityId)throw new Error('施設が選択されていません');const{data,error}=await client().functions.invoke('fetch-gbp-reviews',{body:{facility_id:facilityId}});if(error)throw error;return data as {synced:number}},onSuccess:()=>qc.invalidateQueries({queryKey:['reviews',facilityId]})})
