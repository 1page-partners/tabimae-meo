...
export function AppSidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return <aside className={`sidebar ${open ? 'open' : ''}`}>
    ...
    <div className="facility"><span className="avatar">月</span><div><strong>箱根温泉旅館 月の宿</strong><small>{user?.email ?? 'モックモード'}</small></div><button aria-label="ログアウト" onClick={() => void signOut()}><LogOut size={17} /></button></div>
  </aside>
}
