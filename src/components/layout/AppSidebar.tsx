...
export function AppSidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return <aside className={`sidebar ${open ? 'open' : ''}`}>
    ...
    <nav className="sidebar-nav" aria-label="メインメニュー">{nav.map(({ to, label, sub, icon: Icon }) => <NavLink key={to} to={to} end={to === '/'} onClick={onClose} className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}><Icon size={19} strokeWidth={1.8} /><span className="nav-copy"><strong>{label}</strong><small>{sub}</small></span></NavLink>)}</nav>
